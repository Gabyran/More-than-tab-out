...
/* ----------------------------------------------------------------
   ...
   Data shape stored under the "wallpaper" key:
   {
     type: "url" | "preset" | "upload",   // how it was set
     value: "https://..." | "data:image/..."  // the image source
   }
...
/** Save wallpaper config. Pass null to clear. */
...
async function applyWallpaper(config) {
  ...
  // Extract dominant color and tint cards accordingly
  try {
    const color = await extractDominantColor(config.value);
    if (color) {
      const { r, g, b } = color;
      // Luminance to decide light/dark text
    ...
    console.warn('[tab-out] Could not extract color:', e);
  }
}

/**
 * Extract the dominant color from an image URL.
 * Samples a grid of pixels and returns the average.
 */
  ...
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      try {
        ...
        // Sample every 4th pixel in a grid
        ...
        // CORS tainted canvas — fallback to a neutral tint
        resolve(null);
      }
    };
    ...
    // For preset/URL wallpapers, try loading directly
    // For upload (data: URLs), it works without CORS
    img.src = src;
...
async function renderStaticDashboard() {
  ...
  // Merge subdomains: if multiple hostnames share the same parent domain (e.g. *.bilibili.com),
  // group them under the parent domain name
  function getParentDomain(hostname) {
    const parts = hostname.split('.');
    if (parts.length <= 2) return hostname; // e.g. github.com
  ...
  // Check if a parent domain has multiple different subdomains
  const parentMap = {};
  for (const [key, group] of Object.entries(groupMap)) {
    if (key.startsWith('__')) continue;
    if (group.label) continue; // skip custom groups
    const parent = getParentDomain(group.domain);
    if (!parentMap[parent]) parentMap[parent] = [];
    parentMap[parent].push({ key, group });
  }

  // Merge groups that share a parent domain
  for (const [parent, groups] of Object.entries(parentMap)) {
    if (groups.length < 2) continue; // only merge if 2+ subdomains
    const target = groups[0];
    const mergedTabs = [...target.group.tabs];
    const mergedKeys = [target.key];

    for (let i = 1; i < groups.length; i++) {
      mergedTabs.push(...groups[i].group.tabs);
      delete groupMap[groups[i].key];
      mergedKeys.push(groups[i].key);
    }

    // Use parent domain as key, prefer friendly name
    const friendlyName = friendlyDomain(parent) || parent;
    target.group.domain = parent;
    target.group.tabs = mergedTabs;
    if (target.key !== parent) {
      delete groupMap[target.key];
      groupMap[parent] = target.group;
    }
  }
...
async function renderQuickLinks() {
  ...
  const linksHtml = links.map(link => {
    const faviconUrl = (() => {
      try { return `https://www.google.com/s2/favicons?domain=${new URL(link.url).hostname}&sz=32`; }
      catch { return ''; }
    })();
    ...
    return `
      <a class="quick-link" href="${link.url}" target="_blank" rel="noopener"
         ...
         title="${link.name}">
        <img class="quick-link-favicon" src="${faviconUrl}" alt="" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
...
/* ----------------------------------------------------------------
   WALLPAPER SETTINGS — Render + Logic
   ---------------------------------------------------------------- */

// Preset wallpapers (curated Unsplash photos)
...
async function renderWallpaperPanel() {
  ...
  const activeId = current?.type === 'preset'
    ? PRESET_WALLPAPERS.find(p => p.url === current.value)?.id || ''
    : '';

  const presetsHtml = PRESET_WALLPAPERS.map(p => {
    const isActive = p.id === 'none' ? !current : activeId === p.id;
    const thumbStyle = p.url
      ? `background-image:url('${p.url}');background-size:cover;background-position:center;`
      : `background:var(--warm-gray);`;
    return `
      <div class="wallpaper-preset ${isActive ? 'active' : ''}"
           data-action="select-wallpaper-preset"
           data-preset-id="${p.id}"
           data-preset-url="${p.url}"
           style="${thumbStyle}"
           title="${p.label}">
        ${p.id === 'none' ? '<span class="preset-none-label">✕</span>' : ''}
      </div>`;
  }).join('');
  ...
  panel.innerHTML = `
    ...
    <div class="panel-section">
      <label class="panel-label">Image URL</label>
      <div class="panel-input-row">
        <input type="text" class="panel-input" id="wallpaperUrlInput"
               placeholder="https://images.unsplash.com/..." value="${current?.type === 'url' ? current.value : ''}">
        <button class="panel-btn" data-action="set-wallpaper-url">Apply</button>
      </div>
    </div>
    <div class="panel-section">
      <label class="panel-label">Upload</label>
...
/* ----------------------------------------------------------------
   WALLPAPER + QUICK LINKS — Event Handlers
   ---------------------------------------------------------------- */

// Wallpaper preset click
document.addEventListener('click', async (e) => {
  const preset = e.target.closest('[data-action="select-wallpaper-preset"]');
  if (!preset) return;
  ...
  if (presetId === 'none') {
    ...
    await setWallpaper({ type: 'preset', value: preset.dataset.presetUrl });
...
// Wallpaper URL apply
document.addEventListener('click', async (e) => {
  const btn = e.target.closest('[data-action="set-wallpaper-url"]');
  if (!btn) return;
  ...
  if (!url) {
    ...
    await setWallpaper({ type: 'url', value: url });
    showToast('Wallpaper applied');
  }
  await renderWallpaperPanel();
});
...
document.addEventListener('click', (e) => {
  if (!e.target.closest('[data-action="close-wallpaper-panel"]')) return;
...
// Add quick link
document.addEventListener('click', async (e) => {
  if (!e.target.closest('[data-action="add-quick-link"]')) return;
...
document.addEventListener('click', async (e) => {
  const editBtn = e.target.closest('[data-action="edit-quick-link"]');
  if (!editBtn) return;
