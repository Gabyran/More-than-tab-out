...
async function renderWallpaperPanel() {
  ...
  const linksHtml = links.map(link => {
    ...
    return `
      <div class="panel-link-item" data-link-id="${escapeAttr(link.id)}" data-link-name="${escapeAttr(link.name)}" data-link-url="${escapeAttr(link.url)}">
        <img src="${faviconUrl}" alt="" class="panel-link-favicon" onerror="this.style.display='none'">
        <span class="panel-link-name">${escapeHtml(link.name)}</span>
        <button class="panel-link-edit" data-action="edit-quick-link" data-link-id="${escapeAttr(link.id)}" title="Edit / Delete">
  ...
  panel.innerHTML = `
    ...
    <div class="panel-section">
      ...
      <div class="panel-input-row">
        <input type="text" class="panel-input" id="wallpaperUrlInput" placeholder="https://images.unsplash.com/..." value="${escapeAttr(current?.type === 'url' ? current.value : '')}">
...
async function handleEditQuickLink(actionEl, e) {
  e.preventDefault(); e.stopPropagation();
  const linkId = actionEl.dataset.linkId;
  const linkEl = actionEl.closest('.quick-link') || actionEl.closest('.panel-link-item');
  ...
  if (action === '2') {
    ...
    const newName = prompt('Site name:', currentName);
    if (!newName) return;
    let newUrl = prompt('URL:', currentUrl);
    if (!newUrl) return;
    if (!newUrl.startsWith('http')) newUrl = 'https://' + newUrl;
    await updateQuickLink(linkId, newName, newUrl);
    await renderQuickLinks();
    if (wallpaperPanelOpen) await renderWallpaperPanel();
...
const ACTION_MAP = {
  ...
  'close-wallpaper-panel':  (el, e) => handleCloseWallpaperPanel(),
  'add-quick-link':         (el, e) => handleAddQuickLink(),
  'import-bookmarks':       (el, e) => handleImportBookmarks(),
  'edit-quick-link':        (el, e) => handleEditQuickLink(el, e),
};
